...
public class Configuration {
  ...
  public StatementHandler newStatementHandler(Executor executor, MappedStatement mappedStatement, Object parameterObject, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
    /**委派模式-StatementHandler是Sql构建的处理*/
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    /**mybatis的一个机制，当一个事务中不能单挑插入又批量插入，在下面几行就是确定执行类型，要么单条要么选择批量
     * SIMPLE
     * 这个类型不做特殊的事情，它只为每个语句创建一个PreparedStatement。可以返回自增键，自增键会在事务提交后，自动设置到传入的对象中。
     * REUSE
     * 这种类型将重复使用PreparedStatements。
     * BATCH
     * 这个类型批量更新，性能更优，但batch模式也有自己的问题，比如在Insert操作时，在事务没有提交之前，是没有办法获取到自增的id，
     * 这在某型情形下是不符合业务要求的，而且假如有一条sql语句报错，则整个事务回滚，虽然这条sql语句不是太重要。
     * 注意：在同一事务中batch模式和simple模式之间无法转换。
     */
    ...
    //这个是executor的增强实现 同样继承Executor
    if (cacheEnabled) {//mybatis总配置文件中settings节点中cacheEnabled子节点的值，默认就是true
      /**装饰模式*/
